...
public final class TargetingResolver {
    ...
    private static TargetResult imaginary(ViewRay ray, TargetingSpec spec) {
        if (ray == null) { return TargetResult.fallback(new PointTarget(new Vec3d(0.0, 0.0, 0.0)), spec.fallbackPolicy()); }
        return TargetResult.fallback(new PointTarget(ray.pointAt(spec.rangeBlocks() * 0.5)), spec.fallbackPolicy());
    }
}
