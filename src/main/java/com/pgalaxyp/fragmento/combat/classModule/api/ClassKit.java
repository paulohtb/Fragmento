...
public record ClassKit(ClassId id, Set<WeaponId> weapons, Set<AbilityId> abilities, Map<ActionSlot, AbilityId> defaultLoadout) {
    public ClassKit {
        ...
        if (weapons.isEmpty() || abilities.isEmpty() || defaultLoadout.isEmpty()) throw new IllegalArgumentException();
        ...
        if (!abilities.containsAll(defaultLoadout.values())) throw new IllegalArgumentException();
    }
}