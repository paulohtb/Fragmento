...
public final class NfCodec {
    ...
    private static DomainIntent readIntent(DataInputStream in) throws Exception {
        return switch (in.readInt()) {
            ...
            case INTENT_PERFORM -> new PerformActionIntent(ComboInput.values()[in.readInt()]);
    ...
    private static DomainEvent readEvent(DataInputStream in) throws Exception {
        if (in.readInt() != EVENT_HOMING_MAGIC) throw new IllegalArgumentException();
        ...
        return new HomingMagicVisualEvent(frameId, local, source, target, lifetime);
    }
