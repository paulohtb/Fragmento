...
public final class AbilityCombatEngine implements AbilityCombatPort {
    ...
    public AbilityCombatResult tryExecute(AbilityIntent intent, FrameContext frame, GameState state) {
        ...
        TargetResult target = targeting.resolve(new TargetingContext(actorId, def.targeting(), targeting.world()));
        ActorId resolvedTarget = target.actorTargetOpt().orElse(actorId);
        ...
        return new AbilityCombatResult(
                List.of(new AbilityEvent.Started(snap, def.startEffect(), target, actorId, resolvedTarget)),
                List.of()
        );
    }
    ...
    public AbilityCombatResult tick(FrameContext frame, GameState state) {
        ...
        return events.isEmpty() ? AbilityCombatResult.empty() : new AbilityCombatResult(events, List.of());
    }
    ...
    public AbilityFrameView view(Collection<ActorId> actorIds, long frameId) {
        ...
        return new AbilityFrameView(repo.activeAll(frameId));
    }

    private static AbilityCombatResult reject(ActorId actorId, AbilityId abilityId, AbilityRejectReason reason) {
        return new AbilityCombatResult(List.of(new AbilityEvent.Rejected(actorId, abilityId, reason)), List.of());
    }
}