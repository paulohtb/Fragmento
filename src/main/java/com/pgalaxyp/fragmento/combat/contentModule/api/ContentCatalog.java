...
public record ContentCatalog(Map<AbilityId, AbilityDefinition> abilities, Map<EffectId, DamageSpec> effects, List<AbilityRule> abilityRules, Map<WeaponId, AbilityId> primaryBindings, Map<AbilityId, AbilityTriggerSpec> abilityTriggers, AbilityTuning abilityTuning) {
    ...
    public static ContentCatalog of(List<? extends ContentPack> packs) {
        ...
        var effects = new LinkedHashMap<EffectId, DamageSpec>();
        ...
        for (ContentPack p : packs) {
            ...
            var effectMap = Objects.requireNonNull(p.effects());
            for (var e : effectMap.entrySet()) {
                if (e.getKey() == null || e.getValue() == null) throw new IllegalArgumentException();
                putUnique(effects, e.getKey(), e.getValue(), "effectId");
            }
        ...
        return new ContentCatalog(abilities, effects, rules, primary, triggers, tuning == null ? AbilityTuning.DEFAULT : tuning);
    }
    ...
    private static <T> boolean compat(T a, T b) {
        return a == null || b == null || a.equals(b);
    }

    private static <T> T throwConflict() {
        throw new IllegalStateException("Conflicting abilityTuning");
    }
}