...
import com.pgalaxyp.fragmento.cosmetics.client.render.CosmeticRenderResources;
...
public final class CosmeticsPlayerLayer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    private static final float NEG = Float.intBitsToFloat(0xBF800000);
    ...
    public void render(
        ...
        if (poseStack == null || buffers == null || player == null) return;
    ...
    private static void applySlotOffset(PoseStack poseStack, CosmeticSlot slot) {
        if (slot == CosmeticSlot.HEAD) {
            poseStack.translate(0.0F, PX, NEG * PX * 4.0F);
            return;
        }
        ...
        if (slot == CosmeticSlot.BACK) {
            poseStack.translate(0.0F, PX * 2.0F, NEG * PX * 3.0F);
            return;
        }
        if (slot == CosmeticSlot.WAIST) {
            poseStack.translate(0.0F, NEG * PX, PX * 2.0F);
            return;
        }
        if (slot == CosmeticSlot.HAND) {
