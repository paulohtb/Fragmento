package com.pgalaxyp.fragmento.rpg.platform.minecraft.vfx;
...
public final class FragmentoVfx {
    ...
    public static void accept(List<DomainEvent> events) {
        ...
        for (DomainEvent e : events) {
            if (e instanceof HomingMagicVisualEvent(ActorId sourceActorId, ActorId targetActorId, int lifetimeFrames)) {
    ...
    public static void render(PoseStack poseStack, float partialTick) {
        ...
        VertexConsumer vc = buffers.getBuffer(FragmentoRenderTypes.HOMING_MAGIC);
        ...
        for (LineFx fx : active) {
            ...
            vc.vertex(poseStack.last().pose(), (float) sx, (float) sy, (float) sz).color(r, g, b, (int) (alpha * 255f)).endVertex();
            vc.vertex(poseStack.last().pose(), (float) tx, (float) ty, (float) tz).color(r, g, b, (int) (alpha * 255f)).endVertex();
        }

        buffers.endBatch(FragmentoRenderTypes.HOMING_MAGIC);
    }
    ...
    private static Entity findEntityByActorId(ActorId id) {
        ...
        for (Entity e : mc.level.entitiesForRendering()) {
            ...
            if (MinecraftActorIds.fromUuid(e.getUUID()).equals(id)) {
                return e;
            }
        }
        return null;
