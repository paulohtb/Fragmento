...
public class GuitarProjectile extends AbstractProjectile implements GeoEntity {
    ...
    private static final EntityDataAccessor<Boolean> IS_CHARGED =
            SynchedEntityData.defineId(GuitarProjectile.class, EntityDataSerializers.BOOLEAN);
    ...
    public GuitarProjectile(EntityType<? extends Projectile> type, Level world) {
        super(type, world);
    }

    public GuitarProjectile(Level world, LivingEntity shooter, boolean charged) {
        this(EntitiesRegistry.GUITAR_PROJECTILE.get(), world);
        setOwner(shooter);
        setCharged(charged);
    ...
    protected void onHit(HitResult hitResult) {
        ...
        if (hitResult.getType() == HitResult.Type.ENTITY) {
            ...
            if (hitedEntity instanceof LivingEntity living) {
                ...
                if (isCharged()) {
                    living.addEffect(new MobEffectInstance(EffectsRegistry.CONCUSSION, 40, 0));
                }
            }
        }
